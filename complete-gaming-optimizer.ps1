...
Write-Host "  ðŸŽ® Complete Gaming Optimizer v1.0" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""

# Check if running as admin
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-Host "âŒ ERROR: This script must be run as Administrator!" -ForegroundColor Red
    exit 1
}
...
Write-Host "âœ… FSO Disabled (GameDVR_FSEBehaviorMode = 2)" -ForegroundColor Green
Write-Host "âœ… GameDVR Disabled" -ForegroundColor Green
...
# ============================================
# 1. DISABLE FULLSCREEN OPTIMIZATIONS & GAME DVR
# ============================================
Write-Host "[1/6] Disabling Fullscreen Optimizations & GameDVR..." -ForegroundColor Yellow
...
foreach ($key in $gamedvrSettings.Keys) {
    New-ItemProperty -Path $registryPath -Name $key -PropertyType DWord -Value $gamedvrSettings[$key] -Force | Out-Null
    Write-Host "Set $key = $($gamedvrSettings[$key])" -ForegroundColor Green
}
...
# ============================================
Write-Host ""
Write-Host "[2/6] Disabling CPU Power Saving..." -ForegroundColor Yellow
...
foreach ($service in $servicesToDisable) {
    try {
        ...
        Write-Host "⚠  Skipping: $service (not found or not accessible)" -ForegroundColor Yellow
    }
}
...
if (Test-Path $amdPath) {
    ...
    Write-Host "⚠  AMD GPU not detected" -ForegroundColor Yellow
}
